...
"""Generation-safe copy-on-write prompt-cache branches.

The stored APC snapshot and every restored branch have distinct Python object
graphs, but their MLX arrays are descriptor aliases made with
``mx.stop_gradient``.  MLX keeps those buffers shared until one descriptor is
written.  This preserves the concrete cache classes and their public
interfaces; no proxy object reaches a model.

This module deliberately does not promise that a stock B=2 attention consumer
can read two B=1 branches without materialising a physical batch.  It removes
the eager APC deepcopy and provides independent target/MTP branches.  Avoiding
the later B2 merge still requires a segment-aware consumer.
"""
...
class COWCacheTelemetry:
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        # Spell out the attribution boundary in the receipt itself.
...
def _tree_nbytes(value: Any, seen: Optional[set[int]] = None) -> int:
    ...
    if isinstance(value, dict):
        return sum(_tree_nbytes(k, seen) + _tree_nbytes(v, seen) for k, v in value.items())
...
def _plane_for_cache(cache: Any, *, draft: bool = False) -> str:
    ...
    if "qsa" in name:
        return "qsa_summary"
    if "arrayscache" in name or (hasattr(cache, "cache") and not hasattr(cache, "keys")):
...
def _qsa_plane_projection(
    ...
    compatibility = tuple(
        (name, repr(getattr(cache, name, None))) for name in names
    )
...
def _validate_stable_source(prompt_cache: Iterable[Any]) -> None:
    ...
    while stack:
        ...
        if getattr(cache, "_mtp_share_topk", False) or getattr(
...
class _SegmentToken:
    ...
    def note_mutation(self, reason: str) -> bool:
        # The normal decode path reaches this wrapper on every token. Once a
        # branch has detached, avoid paying a Python lock for every later
        # update; the locked recheck preserves exactly-once accounting.
...
def _attach_mutation_hooks(
    cache: Any, tokens: dict[str, _SegmentToken]
) -> None:
    ...
    # ``ArraysCache.__setitem__`` delegates to its internal list, and Python
    # resolves special methods on the class rather than the instance.  A
    # per-branch list wrapper is therefore the narrow way to account real GDN
    # slot writes without adding a conditional to the global cache hot path.
...
def _clone_graph(
    ...
    *,
    ...
    if not hasattr(value, "__dict__"):
        # Unknown extension objects can contain mutable state invisible here.
        raise COWCacheUnsupported(
    ...
    # CacheList-like containers delegate to already-instrumented children;
    # attaching a second container token would double-count one mutation.
    if (
        attach_tokens
        ...
        if clone_plane == "qsa_summary":
            ...
            tokens = {
                ...
                "qsa_summary": _SegmentToken(
                    telemetry,
                    "qsa_summary",
                    max(0, total_bytes - attention_bytes),
                ),
            }
        else:
            try:
...
class COWCacheOwner:
    ...
    def __init__(
        ...
        *,
        ...
        grouped: dict[
            CachePlaneKind,
            list[tuple[Any, Any, tuple[tuple[str, str], ...]]],
        ] = {}
        for cache in _iter_cache_objects(self._source_cache):
            plane = _plane_for_cache(cache)
            # QSA cache objects carry ordinary attention K/V and an
            # independently-versioned raw/index/pooled summary plane.
        ...
        if source_sidecar is not None:
            grouped[CachePlaneKind.MTP_DRAFT] = [
                (source_sidecar, source_sidecar, ())
            ]
        owners = []
        self._plane_descriptors: dict[
            CachePlaneKind, COWDevicePlaneDescriptor
        ] = {}
    ...
    def invalidate_plane(self, kind: CachePlaneKind, reason: str) -> int | None:
        # Branch admission and required-plane invalidation share one lock.
        # Otherwise a plane could become invalid between branch()'s owner-state
        # check and publication of ``_invalid_planes``.
        with self._lock:
        ...
        try:
            ...
            with self._lock:
                if (
                    ...
                    self.telemetry.add("stale_rejections")
                    raise COWCacheStale(
                        "APC COW source changed before branch adoption"
                    )
                if CachePlaneKind.MTP_DRAFT in self._invalid_planes:
                    sidecar = None
    ...
    def branch_rows(
        ...
        try:
            for _ in range(rows):
                branches.append(
                    self.branch(expected_generation=expected_generation)
                )
        except Exception:
...
class COWPromptCacheBranch(list):
    ...
    def close(self) -> None:
        with self._close_lock:
            ...
            # Explicit close is a lifetime boundary, not merely an owner-pin
            # decrement. Drop every branch-held descriptor and sidecar so a
            # long-lived request wrapper cannot retain device backing.
            self.clear()
    ...
    def __deepcopy__(self, memo: dict[int, Any]) -> "COWPromptCacheBranch":
        ...
        with self._close_lock:
            ...
            # Keep close from clearing the live tip until the successor pin is
            # acquired and its descriptor graph is complete. Reuse deepcopy's
            # memo so the draft cache reachable through SelfMTPCachePair and
            # this branch's sidecar remains one shared cloned object.
    ...
    def fork(self) -> "COWPromptCacheBranch":
        ...
        # A descendant must branch from the immutable owner, never from a
        # possibly-mutated sibling.
...
def note_cache_mutation(cache: Any, reason: str) -> bool:
    """Called by native cache mutators; no-op for legacy cache objects."""

    token = getattr(cache, "_cow_segment_token", None)
    return False if token is None else token.note_mutation(reason)
