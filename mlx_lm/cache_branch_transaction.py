...
"""Exact aligned cache-delta transactions with O(1) branch promotion.

This is the model-independent ownership layer for a future segmented verifier.
It never interprets a cache payload and never forces device evaluation.  A
consumer publishes one aligned delta checkpoint per logical token boundary;
acceptance promotes a checkpoint pointer with one generation-checked swap,
while rejection only retires the branch handle.  Physical compaction is an
explicit two-phase CAS so materialization can run outside the lineage lock.

The feature is default-off.  Existing cache consumers and APC behavior do not
enter this module unless ``MLX_LM_CACHE_DELTA_PROMOTION`` is enabled or a caller
passes ``enabled=True`` to :func:`create_cache_delta_lineage`.
"""
...
class PlaneBase:
    ...
    def __post_init__(self) -> None:
        if min(self.start, self.length, self.generation, self.logical_bytes) < 0:
            raise ValueError("base coordinates, generation, and bytes must be non-negative")
...
class PlaneDelta:
    ...
    def __post_init__(self) -> None:
        if min(self.start, self.length, self.generation, self.logical_bytes) < 0:
            raise ValueError("delta coordinates, generation, and bytes must be non-negative")
        if self.length < 1:
...
class CacheDeltaLineage:
    ...
    def __init__(self, bases: Iterable[PlaneBase]) -> None:
        ...
        self._ring_capacities = {
            base.kind: base.ring_capacity for base in bases
        }
        self._generation = generations.pop()
        self._closed = False
        position = positions.pop()
        self._tip = AlignedDeltaCheckpoint(
            uuid.uuid4().hex, position, (), None, 0, 0
        )
        # Arena ownership makes branch rejection a status flip rather than a
        # recursive Python refcount walk. Old nodes are released at compaction
        # or lineage disposal, off the acceptance critical path.
        self._arena: dict[str, AlignedDeltaCheckpoint] = {
            self._tip.checkpoint_id: self._tip
        }
        self._checkpoint_index: dict[
            tuple[str, int], AlignedDeltaCheckpoint
        ] = {}
    ...
    def promote(
        ...
        with self._lock:
            ...
            abandoned_bytes = (
                branch._tip.cumulative_bytes - checkpoint.cumulative_bytes
            )
            successor = source + 1
    ...
    def reject(self, branch: CacheDeltaBranch) -> RejectionReceipt:
        with self._lock:
            if branch._lineage is not self:
                raise CacheBranchTransactionError(
                    "branch belongs to another lineage"
                )
            if branch._status != BranchStatus.ACTIVE:
                raise CacheBranchTransactionError(
                    f"branch is already {branch._status.value}"
    ...
    def _reject_locked(self, branch: CacheDeltaBranch) -> RejectionReceipt:
        ...
        abandoned_bytes = (
            branch._tip.cumulative_bytes - branch._origin.cumulative_bytes
        )
    ...
    def needs_compaction(
        self, *, max_delta_depth: int, max_delta_bytes: int
    ) -> bool:
    ...
    def commit_compaction(
        self,
        plan: CompactionPlan,
        replacements: Mapping[CachePlaneKind, PlaneBase],
    ) -> CompactionReceipt:
        with self._lock:
            self._assert_open_locked()
            ...
            # An authentic plan is single-use even when replacement validation
            # fails. All authoritative fields below come from the issued record.
            self._issued_compactions.pop(plan.plan_id)
            ...
            for previous in issued_bases:
                ...
                if source_ids.intersection(
                    _payload_object_ids(replacement.payload)
                ):
                    refuse(
                        f"compacted {previous.kind.value} payload aliases source"
                    )
            ...
            new_counters["compaction_retired_empty_branches"] += len(
                empty_branches
            )
            ...
            # Authoritative publication starts only after every allocation and
            # receipt value above succeeded.
            ...
            self._arena_version = 0
            self._counters = new_counters
        # Clearing the old arena can decref an arbitrarily deep payload graph.
        # Keep that O(N) work outside the lineage lock.
        old_index.clear()
        old_arena.clear()
        return receipt
    ...
    def compact(
        self,
        adapter: ExactCompactionAdapter,
    ) -> CompactionReceipt:
        ...
        try:
            replacements = {
                base.kind: materialize(base, deltas[base.kind])
            ...
            result.update(
                {
                    ...
                    "live_delta_bytes": (
                        0 if self._tip is None else self._tip.cumulative_bytes
                    ),
...
__all__ = [
    ...
    "CompactionReceipt",
    "DeltaChainView",
    "ExactCompactionAdapter",
    "PlaneBase",
    "PlaneDelta",
    "PromotionReceipt",
    "RejectionReceipt",
    "cache_delta_promotion_enabled",
