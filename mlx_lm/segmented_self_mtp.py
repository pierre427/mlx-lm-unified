...
"""Host-only receipts for independent-B1 self-MTP cache consumption.

This module does not proxy a model cache.  Concrete request-private B1 caches
continue to reach Qwen4 unchanged; the ledger records their aligned logical
KV/ring/QSA/GDN/MTP boundary and makes publication generation-safe.  It never
reads an array value or synchronizes a device.
"""
...
def _plane_groups(pair: Any) -> dict[CachePlaneKind, tuple[Any, ...]]:
    ...
    if not groups.get(CachePlaneKind.ATTENTION_KV) and not groups.get(
        CachePlaneKind.ATTENTION_RING
    ):
        # A synthetic/model-free recurrent fixture still needs an aligned
        # authoritative target plane. Production Qwen4 always has KV/QSA.
...
def _fingerprint(kind: CachePlaneKind, caches: tuple[Any, ...]) -> tuple[Any, ...]:
    return tuple(
        (
            ...
            tuple((name, repr(getattr(cache, name, None))) for name in _FORMAT_FIELDS),
        )
        for cache in caches
    )
...
def _plane_logical_bytes(kind: CachePlaneKind, caches: tuple[Any, ...]) -> int:
    ...
    for cache in caches:
        ...
        if fields == ("*",):
            ...
            total += _tree_nbytes(tuple(getattr(cache, name, None) for name in fields))
    return total
...
def _group_positions(caches: Iterable[Any]) -> tuple[int, ...]:
    ...
    for cache in caches:
        ...
        if isinstance(offset, Integral) and not isinstance(offset, bool):
            ...
            continue
        # Ignore the inherited default size()=0 for recurrent caches, which
        # explicitly means "position unavailable". A concrete override is a
        # position-bearing contract and must agree with its siblings.
...
def _require_group_position(
    ...
    if positions and any(position != int(expected) for position in positions):
        raise ValueError(
            f"segmented {kind.value} positions {positions} disagree with "
            f"logical coordinate {expected}"
        )
...
def _stamp(
    ...
    for cache in caches:
        is_qsa = "qsa" in type(cache).__name__.lower() or hasattr(
            cache, "index_keys"
        )
        if is_qsa and kind in (
    ...
    physical = _require_group_position(
        kind, caches, expected_physical
    )
    if physical is None:
        raise ValueError(
            "MTP B1 cache plus pending sidecar is not caught up to the "
...
class SegmentedLaneTransaction:
    ...
    def __init__(self, pair: Any, lane: Any, position: int) -> None:
        ...
        if lineage is None:  # pragma: no cover - enabled=True is authoritative
        ...
        *,
        ...
        if advance == 0:
            self.validate(
                ...
                lane,
                stop,
                continuity=not zero_rollback_attested,
            )
            branch.reject()
            note_segmented_self_mtp("transaction_rejections")
            note_segmented_self_mtp("accepted_zero")
            if zero_rollback_attested:
                # Exact core rollback may replace MLX array descriptors even
                # though it restores the same logical/raw state. Retire the
                # branch in O(1), then make that descriptor form the base of
                # an explicitly linked successor lineage.
