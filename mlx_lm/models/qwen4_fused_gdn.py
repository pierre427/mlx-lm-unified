...
"""Default-off fused Qwen4 GDN decode operator.

This module deliberately contains no import-time Metal work.  The kernel is
created lazily and is only eligible for the production B=1, S=1 decode shape.
"""
...
# Single-threadgroup production specialization.  A grid-wide barrier does not
# exist in Metal, so the only correct way to feed all 48 state-owning heads
# directly into one affine-Q4 projection in the same dispatch is to process
# those heads in waves inside one 1024-thread group.  The 6144-wide normalized
# vector lives in threadgroup memory and is never materialized as an MLX array.
_SOURCE_OUTPROJ = r"""
  const uint lane = thread_position_in_threadgroup.x;
  const uint ty = thread_position_in_threadgroup.y;
  const uint tid = thread_index_in_threadgroup;
