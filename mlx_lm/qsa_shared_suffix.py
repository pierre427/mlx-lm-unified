...
"""Default-unused storage ABI for shared QSA prefixes and private suffixes.

The normal QSA cache stores ``[prefix, live suffix]`` in every request row.
This module keeps the prefix in one immutable object and gives each row a
small physical KV cache plus raw-index ledger.  It deliberately does not
pretend to be a stock ``QSAKVCache``: crossing back to a stock consumer is an
explicit, receipted materialization boundary.
"""
...
class SharedSuffixQSAKVCache:
    ...
    def from_cache(
        ...
        *,
        ...
        if suffix:
            row.append_index_keys(
                cache.index_keys[:, base.length : cache.offset]
            )
            row.append_kv(
    ...
    def release_qsa_cycle(self, who: str, *, keep_pooled: bool = True) -> None:
        ...
        if ledger < self._kv.offset:
            raise SharedSuffixQSAError(
                f"{who}: the suffix raw-key ledger holds {ledger} positions "
                f"but suffix K/V holds {self._kv.offset}"
            )
        if ledger > self._kv.offset:
    ...
    def append_index_keys(self, keys: mx.array) -> mx.array:
        ...
        self.index_keys = (
            ...
            if self.index_keys is None
                ...
                [self.index_keys[:, : self._kv.offset], keys], axis=1
            )
        )
    ...
    def append_kv(
        ...
        *,
        ...
        if ledger_width != expected:
            # Qwen4's shared-top-k draft step intentionally omits transient
            # raw index keys.  Admit that state only when the caller names it;
            # materialization remains forbidden until the KV is rewound to the
            # ledger or a later exact append catches it up.
    ...
    def update_index_keys(self, keys: mx.array):
        raise SharedSuffixQSAError(
            "shared-suffix QSA requires the split-aware indexer"
        )
    ...
    def set_suffix_pooled_keys(self, pooled: mx.array | None) -> None:
        ...
        if pooled is not None:
            ...
            # The indexer runs before the attention KV append.  During that
            # interval the raw ledger legitimately leads physical suffix K/V
            # by the current query width, just as it does in stock QSA.
            ledger = 0 if self.index_keys is None else int(self.index_keys.shape[1])
    ...
    def materialize_to_qsa(
        self,
    ) -> tuple[QSAKVCache, QSAMaterializationReceipt]:
        ...
        if self.index_keys is None:
            ...
            index_keys = mx.concatenate(
                [self.base.index_keys, self.index_keys], axis=1
            )
        ...
        return cache, QSAMaterializationReceipt(
...
def split_attested_qsa_rows(
    ...
    *,
    ...
    base = QSAImmutableBase.from_cache(
        first, layout_id=layout_id, length=base_length
    )
    ...
    rows = [SharedSuffixQSAKVCache.from_cache(cache, base, note=note) for cache in caches]
    if note is not None:
