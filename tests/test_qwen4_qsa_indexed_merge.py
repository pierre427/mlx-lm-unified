...
class TestQSAIndexedMerge(unittest.TestCase):
    ...
    def test_sequential_order_is_independent_of_split_grouping(self):
        m, l, o = self.partials()
        one_split = merge.mlx_sequential_merge(
            m.reshape(1, 1, 1, 1, 8),
            l.reshape(1, 1, 1, 1, 8),
            o.reshape(1, 1, 1, 1, 8, 256),
            output_dtype=mx.float32,
        )
        two_splits = merge.mlx_sequential_merge(
            m.reshape(1, 1, 1, 2, 4),
            l.reshape(1, 1, 1, 2, 4),
            o.reshape(1, 1, 1, 2, 4, 256),
            output_dtype=mx.float32,
        )
        reversed_chunks = merge.mlx_sequential_merge(
            m[..., ::-1],
            l[..., ::-1],
            o[..., ::-1, :],
            output_dtype=mx.float32,
    ...
    def test_env_unset_keeps_the_fused_dispatch_inert_and_output_identical(self):
        ...
        with (
            ...
            mock.patch.object(
                merge,
                "_fused_merge",
                side_effect=AssertionError("fused dispatch must stay inert"),
            ),
        ):
        ...
        with (
            mock.patch.dict(os.environ, env, clear=True),
            mock.patch.object(indexed, "_combine_kernel", return_value=old_kernel),
            mock.patch.object(
                indexed,
                "combine_indexed_partials",
                side_effect=AssertionError("env-unset hook changed dispatch"),
            ),
        ):
            hooked = indexed._combine_sdpa_partials(
                hook_m, hook_m, hook_o, output_dtype=mx.float32
            )
    ...
    def test_runtime_error_falls_back_and_records_merge_receipt(self):
        ...
        with (
            mock.patch.dict(
                os.environ,
                {"MLX_QWEN4_QSA_INDEXED_FUSED_MERGE": "1"},
            ),
            mock.patch.object(merge, "fused_merge_available", return_value=True),
            mock.patch.object(
                merge, "_fused_merge", side_effect=RuntimeError("compile")
            ),
        ):
            actual = indexed._combine_sdpa_partials(m, l, o, output_dtype=mx.float32)
        mx.eval(expected, actual)
        self.assertTrue(mx.array_equal(expected, actual).item())
